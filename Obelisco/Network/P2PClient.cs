...
public class P2PClient : P2P
{
    ...
    protected override async ValueTask GetServersResponse(CancellationToken cancellationToken)
    {
        try
        {
            var servers = m_client.Servers;
    ...
    protected override async ValueTask PostServersResponse(PostServersRequest request, CancellationToken cancellationToken)
    {
        await SendOkResponse(cancellationToken);
        m_postServers?.Invoke(this, request);
    }
    ...
    public async ValueTask<Transaction> GetTransaction(string transactionSignature, bool pending, CancellationToken cancellationToken)
    {
        await SendMessage(new GetTransactionRequest() { TransactionSignature = transactionSignature, Pending = pending }, cancellationToken);
        return WaitResponse<TransactionResponse>(cancellationToken).Transaction;
    }
